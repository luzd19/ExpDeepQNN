        ...
        for p in range(size_train):
            ...
            for i in range(num_layer):
                for k in range(num_varia_perceptron):
                    X_s = X + 0; X_s[k, i] = X[k, i] + pi ;
                    partial_U = U( X_s[:, i:i+1] ) ;
                    U_dag = block[i].dag() ;
                    rho = tensor( phiout[i], ket2dm( basis(2, 0) ) ) ;
                    back_term = tensor( qeye(2), sigma[i+1] ) ;
                        
                    gra_x[k,i] = gra_x[k,i] + real( (partial_U * rho * U_dag * back_term ).tr()  ) ;  
